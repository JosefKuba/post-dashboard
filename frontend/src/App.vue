...
<div class="app" :style="appStyle">
  ...
  <template v-else>
    ...
    <section v-if="!isQueryPage" class="filters">
    ...
    <section class="view-tabs">
      ...
      <button v-if="isQueryPage" @click="copyCurrentResults">复制当前结果</button>
      <span>共 {{total}} 条</span>
        ...
        <div class="drawer-top">
          ...
          <div class="drawer-summary">
            <div class="drawer-title-row">
              <Avatar :src="detail.page_avatar" :label="detail.page_code"/>
              <div class="drawer-title-text">
                <b>{{detail.page_code||'—'}}</b>
                <span class="muted">{{detail.admin_name||'—'}}</span>
              </div>
            </div>
            <div class="detail-stats">
              <span><em>引流</em>{{detail.leads??0}}</span>
              <span><em>邀约</em>{{detail.invites??0}}</span>
              <span><em>上线</em>{{detail.online??0}}</span>
              <span><em>交教会</em>{{detail.church??0}}</span>
            </div>
            <div class="detail-meta">
              <div><label>引流日</label><span>{{formatDate(detail.lead_date)||'—'}}</span></div>
              <div><label>发帖日</label><span>{{formatDate(detail.post_time)||'—'}}</span></div>
              <div><label>男女比</label><span>{{genderRatio(detail)}}</span></div>
              <div><label>类型</label><span>{{detail.page_post_type||'—'}}</span></div>
              ...
              <div class="full" v-if="detail.summary_source_name||detail.summary_source_sheet"><label>来源</label><span>{{detail.summary_source_name}} / {{detail.summary_source_sheet}}</span></div>
            </div>
          </div>
        </div>
        <!-- 下：文案区块（始终展示，空内容显示占位） -->
...
const mainPages=[{key:'posts',title:'引流帖文'},{key:'leads',title:'引流排行榜'},{key:'invitesOverview',title:'邀约帖文'},{key:'onlineOverview',title:'上线帖文'},{key:'church',title:'交教会帖文'}]
...
const baseColumns=[
  ...
  {key:'page_post_type',label:'类型',type:'text',width:140,align:'left',order:15,visible:false,detailOnly:false,displayMode:'single',maxLines:1},
...
const COPYABLE_KEYS=new Set(['post_ocr_translation','video_translation'])
...
async function init(){await loadUi();restoreLocal();normalizeUi();await loadAdmins();await loadPages();if(isQueryPage.value){if(queryText.value.trim())await runQuery()}else await loadData()}
...
function normalizeUi(){
  ...
  ui.columns=ui.columns.filter(c=>c&&!hiddenColumnKeys.has(c.key))
